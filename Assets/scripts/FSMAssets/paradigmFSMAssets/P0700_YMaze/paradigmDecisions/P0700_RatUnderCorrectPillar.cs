...
{
    [CreateAssetMenu(menuName = "FSM/Decisions/P0700/P0700_RatUnderCorrectPillar")]
    public class P0700_RatUnderCorrectPillar : Decision
    {
        ...
        public override bool Decide(BaseStateMachine stateMachine)
        {
            ...
            for (int i = 0; i < childcount; i++)
            {
                ...
                if (child.GetComponentInChildren<PillarCollision>().PlayerDetected)
                {
                    ...
                    {
                        Debug.Log("player hits the correct pillar" + stateMachine._sceneController.scene.Pillars[i].UID);
                        return true;
                    }
                    else
                    {
